...
FILE * wavfile_open( const char *filename )
{
	...
	header.num_channels = WAVFILE_CHANNELS;
	header.sample_rate = samples_per_second;
	header.byte_rate = samples_per_second*(bits_per_sample * header.num_channels/8);
	header.block_align = bits_per_sample * header.num_channels/8;
